...
public class FailChallengeTest
    extends AbstractHamsterTest
{
    ...
    @Test
    public void failChallenge() throws DuplicateVoteException
    {
        ...
        Vote v1 = new VoteBuilder().challenge(instance).positive(false).user(voter1).build();
        vs.newVote(v1);
        Vote v2 = new VoteBuilder().challenge(instance).positive(false).user(voter2).build();
        vs.newVote(v2);
        Vote v3 = new VoteBuilder().challenge(instance).positive(true).user(voter3).build();
        vs.newVote(v3);
        ...
        instance = is.findOne(instance.getId());
        assertEquals(ChallengeStatus.FAILED, instance.getStatus());
        ...
        assertEquals(targetMonthlyPoints, target.getCurrentMonthlyPoints());
        assertEquals(targetPermanetnPoints, target.getPermanentPoints());
        assertEquals(targetCreds, target.getCredits());
